...
- `apps/api/Features/AI/OnboardingOrchestratorService.cs` - שאלת העמקה דינמית
- `apps/api/Features/AI/OnboardingPlannerService.cs` - תכנון אונבורדינג חכם
...
**שלב 1 - Preview מיידי (חינם, אוטומטי)**
- מיד אחרי הbuild המשתמש רואה preview בתוך הדשבורד
- כתובת: `ai-business-os.com/preview/[profileId]`
- לא צריך דומיין, לא צריך לשלם
- יכול לבקש שינויים לפני פרסום

**שלב 2 - Subdomain שלנו (חינם)**
- `[שם-עסק].ai-business-os.com`
- עובד מיידית, ניתן לשיתוף
- תמיד חינמי

**שלב 3 - דומיין אישי (בתשלום)**
- המשתמש קונה דומיין (~₪50/שנה)
- אנחנו מחברים אוטומטית
- בעתיד - Reseller API לרכישה דרכנו

### מסך בחירת פרסום (אחרי Preview)
"איך תרצה לפרסם את האתר?"
① פרסם בחינם → ronit-itzuvim.ai-business-os.com

② קנה דומיין אישי → ronit-itzuvim.co.il (₪49/שנה)

③ יש לי דומיין → חבר דומיין קיים

### טכנולוגיה
- Hosting: Vercel (חינמי, deploy אוטומטי)
- Subdomain: Vercel Wildcard Subdomains
- דומיין אישי: Namecheap/GoDaddy + הדרכה לחיבור

### האונבורדינג המלא - Flow סופי

1. שם העסק
2. סוג העסק (כרטיסיות)
3. שאלת העמקה AI (שאלה אחת חכמה)
4. קטגוריות (AI מציע, משתמש מאשר/מוסיף/מוחק)
5. תמונות (Smart Asset Collection לפי קטגוריות)
6. פרטי יצירת קשר (טלפון/WhatsApp חובה, כתובת/שעות אופציונלי)
7. Google Business / אתר קיים? (ייבוא אוטומטי - WOW moment)
8. בחירת סגנון (AI מציע 3 פלטות צבעים, משתמש בוחר)

**מסך Loading:**
- "בונה את האתר שלך..."
- Progress bar עם שלבים: ניתוח עסק → כתיבת תוכן → עיצוב → SEO
- זמן: 5-8 דקות כולל
...
**מה הושלם ב-Sprint 5+6:**
- ✅ UserId מחובר ל-Business Profile
- ✅ GET /api/business-profile endpoint
- ✅ Dashboard מציג נתונים אמיתיים
- ✅ Google Auth מלא
- ✅ Dynamic Deep Question endpoint
- ✅ DeepQuestionScreen component
- ✅ CategoriesScreen component
- ✅ OnboardingPlannerService - AI מחליט מה לשאול
- ✅ BusinessKnowledgeBase - מאגר ידע ל-50+ סוגי עסקים בישראל
- ✅ Context Chaining מלא - כל תשובה משפיעה על הבאה
- ✅ BusinessInsights - צבעים, טון, קהל יעד לכל עסק

**ארכיטקטורת האונבורדינג הנוכחית:**
1. שם עסק (OnboardingScreen)
2. סוג עסק (BusinessTypeScreen)
3. שאלת העמקה - AI (DeepQuestionScreen)
4. קטגוריות - AI מציע (CategoriesScreen) - רק אם רלוונטי
5. Welcome screen עם הודעה אישית מ-Claude

**Business Knowledge Base מכסה:**
- מזון ומסעדות (מסעדה, בית קפה, מאפייה, קייטרינג)
- קמעונאות (בגדים, רהיטים, וילונות, אלקטרוניקה, תכשיטים, חיות, פרחים, יודאיקה)
- שירותים לבית (אינסטלטור, חשמלאי, קבלן, ניקיון, גינון, מנעולן, הדברה, הובלות)
- יופי ובריאות (מספרה, ספר, ציפורניים, ספא, קוסמטיקה)
- שירותים מקצועיים (עורך דין, רואה חשבון, יועץ פיננסי, אדריכל, מעצב פנים, מתווך)
- רפואה ובריאות (פיזיותרפיסט, פסיכולוג, דיאטנית, רופא שיניים, וטרינר)
- חינוך (מורה פרטי, מורה למוזיקה, בית ספר לנהיגה, גן ילדים, יוגה/פילאטיס)
- יצירה ואירועים (צלם, צלם וידאו, מארגן אירועים, מעצב גרפי)
- רכב (מוסך, פחחות)
- דיגיטל (מפתח אתרים, שיווק דיגיטלי, תיקון מחשבים)
- אחר (תרגום, סוכן נסיעות, מדריך טיולים)

**הצעד הבא - Sprint 7:**
- שמירת OnboardingPlan ב-Business Profile ב-MongoDB
- הצגת nextQuestion מה-Plan לאחר קטגוריות
- שאלות פרטי קשר (טלפון, כתובת)
- העלאת תמונות - Smart Asset Collection
- Website Agent - בניית אתר ראשון
